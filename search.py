...
def _dedup_key(link: str) -> str:
    """Normalize a result URL into a de-duplication key.

    Scheme-insensitive with a lower-cased host and trailing slash trimmed, so
    ``http://X.onion``, ``https://X.onion/`` and ``HTTP://x.onion`` collapse to
    a single entry. The path is preserved (only its trailing slash is dropped)
    since onion paths can be case-sensitive.
    """
    link = (link or "").strip()
