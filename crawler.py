"""
crawler.py
Deep Crawl engine for OBSCURA — replaces loader.py + html_txt.py + automated_loader.py.
...
Tier 2 (fallback): requests + SOCKS5 Tor proxy
    Uses OBSCURA's existing scrape.py infrastructure — no extra deps needed.
...
that plugs directly into OBSCURA's existing filter_results → generate_summary
...
def selenium_subtier() -> str:
    ...
    Return a human-readable label for which selenium variant will run:
      ...
      - 'firefox+socks' : stock Firefox routed through 127.0.0.1:9150
...
def _find_socks_port() -> Optional[int]:
    """Return the detected Tor SOCKS port from config (probed lazily)."""
    return obscura_config.get_socks_port()
