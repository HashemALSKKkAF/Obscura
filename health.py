#health.py - Health checks for Tor proxy, LLM connectivity, and search engine availability in OBSCURA OSINT tool.
import time
import socket
import random
...
from tor_session import get_tor_session
from llm import get_llm
from llm_utils import resolve_model_config


def check_tor_proxy():
    """Test that the Tor SOCKS5 proxy is accepting connections."""
    try:
        start = time.time()
        sock = socket.create_connection(("127.0.0.1", obscura_config.get_socks_port()), timeout=5)
        sock.close()
...
def check_llm_health(model_choice):
    ...
    # Display label for the resolved model (handles dynamic local models too).
    provider = providers.label_for_resolved(config["class"], config.get("constructor_params", {}) or {})
